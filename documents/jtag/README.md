...
EBAZ4205のJTAGは3.3VなのでFT2232Dと電圧レベルが一致していますが、電源ノイズや電位差による問題を回避するため、レベル変換ICの使用を推奨します。

**JTAG用途推奨製品（秋月電子）:**

1. **4ビット双方向ロジックレベル変換モジュール BSS138使用** ⭐最小構成向け
   - 商品番号: [M-05452](https://akizukidenshi.com/catalog/g/g113837/) (113837)
   - 価格: 200円
   - 仕様: BSS138 MOSFET使用、4チャンネル双方向
   - 電圧範囲: 1.8V～5.5V
   - 用途: JTAG 4信号（TCK, TDI, TDO, TMS）専用
   - メリット: 最安価、コンパクト、JTAGに必要最小限

2. **8ビット双方向ロジックレベル変換ブレークアウトモジュールキット** ⭐標準構成推奨
   - 商品番号: [AE-LLCNV8-DIP20](https://akizukidenshi.com/catalog/g/g117062/) (117062)
   - 価格: 250円
   - 仕様: 8チャンネル双方向、DIP20相当
   - 在庫: 豊富
   - 用途: JTAG + VCCO_ON検出 + 予備チャンネル
   - メリット: バランスが良い、在庫豊富

3. **LVC8T245使用8ビット双方向レベル変換モジュール** ⭐高信頼性
   - 商品番号: [AE-LLCNV-LVC8T245](https://akizukidenshi.com/catalog/g/g115778/) (115778)
   - 価格: 250円
   - 仕様: TI製 SN74LVC8T245使用、8チャンネル
   - 用途: JTAG + Zynqリセット信号対応
   - メリット: TI製ICで信頼性が高い

4. **8ビット双方向ロジックレベル変換モジュール (FXMA108)**
   - 商品番号: [AE-LLCNV8](https://akizukidenshi.com/catalog/g/g104522/) (104522)
   - 価格: 350円
   - 仕様: FXMA108使用、自動方向検出、8チャンネル
   - 電圧範囲: 1.65V～5.5V（最大7V）
   - 注意: 在庫僅少（要確認）

5. **LVCH16T245使用16ビット双方向レベル変換モジュール** （拡張性重視）
   - 商品番号: [AE-LLCNV-LVCH16T245](https://akizukidenshi.com/catalog/g/g115696/) (115696)
   - 価格: 300円
   - 仕様: 16チャンネル双方向
   - 用途: 複数デバイスのJTAGチェーン接続時

#### EBAZ4205 JTAG用の推奨構成

EBAZ4205のJTAGは3.3Vなので、用途に応じて以下の構成を推奨します：

**構成1: 最小構成（コスト重視）- 約2,200円**
- FT2232D USBシリアル2ch変換モジュールキット ([116897](https://akizukidenshi.com/catalog/g/g116897/)) - 2,000円
- 4ビット双方向ロジックレベル変換モジュール BSS138 ([113837](https://akizukidenshi.com/catalog/g/g113837/)) - 200円
- **用途**: JTAG 4信号のみ使用（TCK, TDI, TDO, TMS）
- **メリット**: 最安価、シンプル
- **デメリット**: リセット信号や電源検出なし

**構成2: 標準構成（バランス重視）- 約2,250円** ⭐推奨
- FT2232D USBシリアル2ch変換モジュールキット ([116897](https://akizukidenshi.com/catalog/g/g116897/)) - 2,000円
- 8ビット双方向レベル変換ブレークアウトモジュールキット ([117062](https://akizukidenshi.com/catalog/g/g117062/)) - 250円
- **用途**: JTAG 4信号 + VCCO_ON電源検出 + 予備チャンネル
- **メリット**: バランスが良い、在庫豊富、将来の拡張に対応
- **推奨理由**: 価格と機能のバランスが最適

**構成3: 完全構成（Zynq用、高信頼性）- 約2,250円**
- FT2232D USBシリアル2ch変換モジュールキット ([116897](https://akizukidenshi.com/catalog/g/g116897/)) - 2,000円
- LVC8T245使用8ビット双方向レベル変換モジュール ([115778](https://akizukidenshi.com/catalog/g/g115778/)) - 250円
- オープンコレクタバッファ（オプション）- ZynqのPS_POR_B/PS_SRST_B信号用
- **用途**: JTAG + Zynqリセット信号対応
- **メリット**: TI製ICで信頼性が高い、Zynq PS制御に対応
- **推奨対象**: Zynq開発で完全な制御が必要な場合

**直接接続（レベル変換ICなし）- 約2,000円**
- FT2232D USBシリアル2ch変換モジュールキット ([116897](https://akizukidenshi.com/catalog/g/g116897/)) - 2,000円のみ
- **条件**: FT2232D (3.3V) とEBAZ4205 JTAG (3.3V) は電圧が一致
- **可能**: 技術的には直接接続可能
- **非推奨**: 電源ノイズや電位差による誤動作のリスクあり
- **推奨**: テスト目的のみ、製品開発には構成2以上を使用

#### オープンコレクタバッファ（Zynq用リセット信号）
- ZynqのPS_POR_B/PS_SRST_B信号用（必要に応じて）
  - NC7WZ07 または SN74LVC07A
